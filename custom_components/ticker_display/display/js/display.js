...
class BridgeWrapper {
    ...
    setScreenOrientation(degrees) {
        var _a;
        if ((_a = this._bridge) === null || _a === void 0 ? void 0 : _a.setScreenOrientation)
            this._bridge.setScreenOrientation(Math.round(degrees));
    }
    ...
    setNativeSetting(setting, value) {
        var _a;
        if ((_a = this._bridge) === null || _a === void 0 ? void 0 : _a.setDeviceSetting) {
            try {
                return this._bridge.setDeviceSetting(String(setting), String(value));
            }
            catch (e) { }
        }
        return false;
    ...
    restartApp() {
        var _a;
        try { (_a = this._bridge) === null || _a === void 0 ? void 0 : _a.restartApp(); }
        catch (e) { }
    }
    openAndroidSettings() {
        var _a;
        try { (_a = this._bridge) === null || _a === void 0 ? void 0 : _a.openAndroidSettings(); }
        catch (e) { }
    }
    reportDeviceStateNow() {
        var _a;
        try { (_a = this._bridge) === null || _a === void 0 ? void 0 : _a.reportDeviceStateNow(); }
        catch (e) { }
    }
    vibrate(ms = 500) {
        var _a;
...
class WebSocketClient {
            ...
            try {
                ...
                ws.onclose = (event) => {
                    ...
                    if ((_a = this.app) === null || _a === void 0 ? void 0 : _a.isPreview) {
                        ...
                        this.app._setOffline("Live-Verbindung unterbrochen", "Automatischer Neuversuch läuft.");
                    }
    ...
    _scheduleReconnect() {
        ...
        if (this._reconnectTimer)
            return;
        try {
            this.app._setOffline("Live-Verbindung unterbrochen", `Nächster Neuversuch in ${Math.round(this._reconnectDelay / 1000)} Sekunden.`);
        } catch (e) { }
...
class ScreenManager {
    ...
    _applyViewportMetrics() {
        try {
            if (typeof window.TickerDisplayApplyViewport === "function")
                window.TickerDisplayApplyViewport();
        }
        catch (e) { }
        const root = document.documentElement;
    ...
    _startBackgroundCarousel(screen, config, images) {
        ...
        if (!pool.length) {
            if (config.background_image)
                this._applyScreenBackground(screen, config, config.background_image);
            return;
        }
    ...
    _fetchLocalBackgroundPool(screen, config) {
        ...
        fetch("/ticker-display/api/media/images", { credentials: "same-origin", cache: "no-store" })
            ...
            .then((items) => {
            const pool = (Array.isArray(items) ? items : []).map(function (img) { return img && img.url ? img.url : ""; }).filter(Boolean);
            if (pool.length)
                this._startBackgroundCarousel(screen, config, pool);
            else
                fallback();
        })
    ...
    _applyScreenStyle(screen, config) {
        ...
        if (carousel) {
            ...
            if ((config.background_carousel_source || "all_local") === "all_local") {
                this._fetchLocalBackgroundPool(screen, config);
                return;
            }
        }
        if (config.background_image)
    ...
    _clearIntervals() {
        ...
        this._backgroundIntervals.forEach(clearInterval);
...
class TickerManager {
    ...
    _applyStyle(cfg = {}) {
        ...
        if (this.bar) {
            this.bar.classList.toggle("top", pos === "top");
            this.bar.classList.toggle("bottom", pos !== "top");
        }
    }
    ...
    _setBarVisible(visible) {
        ...
        const root = document.documentElement;
        if (root)
            root.style.setProperty("--td-ticker-offset", visible ? "var(--td-ticker-height, 36px)" : "0px");
        if (this.bar) {
        ...
        try {
            if (this.app && this.app.screenManager)
                this.app.screenManager._fitCurrentDashboard();
        }
        catch (e) { }
    }
...
class TickerDisplayApp {
    ...
    _showLoadingError(error) {
        const loading = document.getElementById("loading-screen");
        const msg = error && error.message ? error.message : String(error || "Unbekannter Fehler");
        if (loading) {
    ...
    onCommand(cmd, data) {
        ...
        if (cmd === "clear_alert")
            ...
            if (this.screenManager && this.screenManager._openWidgetPopup)
                this.screenManager._openWidgetPopup(cfg);
        }
        else if (cmd === "dismiss_popup") {
            if (this.screenManager && this.screenManager._closeWidgetPopup)
                this.screenManager._closeWidgetPopup();
        }
        else if (cmd === "update_ticker_config") {
            this.config.ticker = { ...(this.config.ticker || {}), ...(data || {}) };
            if (this.tickerManager && this.tickerManager.rebuild)
    ...
    onDisplayControl(data) { if (data.brightness !== undefined)
        ...
        this.bridge.setScreenPower(data.screen_power); if (data.orientation !== undefined)
        this.bridge.setScreenOrientation(data.orientation); }
    async onAudio(data) {
    ...
    onNativeControl(data) {
        var _a, _b;
        const setting = (data && (data.setting || data.key || data.command)) || "";
        const value = data && data.value !== undefined ? data.value : data === null || data === void 0 ? void 0 : data.enabled;
        if (!setting)
            return;
        if (setting === "screen_power")
            this.bridge.setScreenPower(!!value);
        else if (setting === "screen_brightness" || setting === "brightness")
            this.bridge.setScreenBrightness(value);
        else if (setting === "volume" || setting === "media_volume" || setting === "volume_percent")
            this.bridge.setVolume(value);
        (_b = (_a = this.bridge) === null || _a === void 0 ? void 0 : _a.setNativeSetting) === null || _b === void 0 ? void 0 : _b.call(_a, setting, value);
    }

    onNativeAction(data) {
        var _a, _b, _c, _d;
        const action = (data && (data.action || data.command)) || "";
        if (action === "restart_app")
            (_b = (_a = this.bridge) === null || _a === void 0 ? void 0 : _a.restartApp) === null || _b === void 0 ? void 0 : _b.call(_a);
        else if (action === "open_android_settings")
            (_d = (_c = this.bridge) === null || _c === void 0 ? void 0 : _c.openAndroidSettings) === null || _d === void 0 ? void 0 : _d.call(_c);
        else if (action === "vibrate")
            this.bridge.vibrate((data && data.duration) || 500);
        else if (action === "report_now")
            this.bridge.reportDeviceStateNow();
    }
    ...
    onConfigChanged(cfg) {
        ...
        this.tickerManager.rebuild();
        this._startStatePolling();
        if (this.isPreview || ((_a = this.wsClient) === null || _a === void 0 ? void 0 : _a.isConnected()))
            this._setOffline("", "", true);
        try {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.tickerApp.init().catch((e) => {
        console.error("Ticker Display start failed", e);
        if (window.tickerApp && window.tickerApp._showLoadingError)
            window.tickerApp._showLoadingError(e);
    });
});
