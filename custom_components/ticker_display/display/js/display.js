...
// ══════════════════════════════════════════════════════════
  ...
  _syncWidgetToggleBadge(widget, config) {
    if ((config?.tap_action || "none") !== "toggle" || config?.toggle_badge === false) { const ex = widget.querySelector(".widget-toggle-badge"); if (ex) ex.remove(); return; }
  ...
  _openWidgetPopup(config) {
    const overlay = this._widgetPopupOverlay(); const body = overlay.querySelector(".widget-popup-body");
    ...
    if (kind === "weather") html = this._popupWeatherMarkup(config, st);
      ...
      const isOn = Utils.isTruthyState(st.state); const bri = Number(st.attributes?.brightness ?? 0); const briPct = bri ? Math.round((bri / 255) * 100) : 0;
      ...
      const pos = st.attributes?.current_position; const pct = pos == null ? 0 : Math.max(0, Math.min(100, Math.round(Number(pos))));
    ...
    if (controls && domain === "media_player") {
      ...
      this._renderPopupControlButton(controls, "−", false, async () => { await this.app.callEntityService("media_player", "volume_set", { entity_id: entityId, volume_level: Math.max(0, Number(st.attributes?.volume_level ?? 0) - 0.1) }); });
      this._renderPopupControlButton(controls, "+", false, async () => { await this.app.callEntityService("media_player", "volume_set", { entity_id: entityId, volume_level: Math.min(1, Number(st.attributes?.volume_level ?? 0) + 0.1) }); });
      ...
      this._renderPopupControlButton(controls, "− Helligkeit", false, async () => { await this.app.callEntityService('light', 'turn_on', { entity_id: entityId, brightness_pct: Math.max(1, Math.round((Number(st.attributes?.brightness ?? 128) / 255) * 100) - 15) }); close(); });
      this._renderPopupControlButton(controls, "+ Helligkeit", false, async () => { await this.app.callEntityService('light', 'turn_on', { entity_id: entityId, brightness_pct: Math.min(100, Math.round((Number(st.attributes?.brightness ?? 128) / 255) * 100) + 15) }); close(); });
      ...
      this._renderPopupControlButton(controls, "−1°", false, async () => { await this.app.callEntityService("climate", "set_temperature", { entity_id: entityId, temperature: Number(st.attributes?.temperature ?? 20) - 1 }); close(); });
      this._renderPopupControlButton(controls, "+1°", false, async () => { await this.app.callEntityService("climate", "set_temperature", { entity_id: entityId, temperature: Number(st.attributes?.temperature ?? 20) + 1 }); close(); });
