...
(function () {
  ...
  function screenToPage(screen, index = 0) {
    ...
    return {
      ...
      fit_mode: cfg.fit_mode || cfg.fitMode || cfg.viewport_mode || "device-fit",
      viewport_width: cleanInt(cfg.viewport_width || cfg.desktop_width || cfg.design_width, 1644, 320, 4096),
      viewport_height: cleanInt(cfg.viewport_height || cfg.desktop_height || cfg.design_height, 866, 240, 4096),
      force_desktop_scaling: !!cfg.force_desktop_scaling,
    };
  }
  function pageToScreen(page, index = 0) {
    ...
    return {
      ...
      fit_mode: page.fit_mode || "device-fit",
      viewport_width: cleanInt(page.viewport_width, 1644, 320, 4096),
      viewport_height: cleanInt(page.viewport_height, 866, 240, 4096),
      force_desktop_scaling: page.fit_mode === "desktop-fit" || page.fit_mode === "desktop-fill",
    };
  }
  ...
  class TickerDisplayPanel extends HTMLElement {
    ...
    async _loadSelectedDevice() {
      if (!this._selectedId) { this._device = null; this._pages = []; this._modules = defaultModules(); return; }
    ...
    async _runTest(kind) {
      if (!this._device) { this._setMessage("Bitte zuerst ein Gerät auswählen.", true); return; }
      this._setMessage(`Sende Test: ${kind}...`);
      try {
        await this._fetchJson(`${API}/api/device/${encodeURIComponent(this._device.id)}/test`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ test: kind, duration: 4 }) });
        this._setMessage(`Test gesendet: ${kind}`);
      } catch (err) { this._setMessage(`Test fehlgeschlagen: ${err.message || err}`, true); }
    }
    ...
    _updatePage(index, key, value) {
      ...
      if (["duration"].includes(key)) this._pages[index][key] = cleanInt(value, 60, 5, 86400);
      else if (["viewport_width", "viewport_height"].includes(key)) this._pages[index][key] = cleanInt(value, key === "viewport_width" ? 1644 : 866, key === "viewport_width" ? 320 : 240, 4096);
      else if (key === "enabled" || key === "kiosk") this._pages[index][key] = !!value;
      else this._pages[index][key] = value;
      this._render();
    }
    ...
    _onClick(ev) {
      ...
      else if (action === "test") this._runTest(target.dataset.test || "selftest");
    }
    _onInput(ev) {
